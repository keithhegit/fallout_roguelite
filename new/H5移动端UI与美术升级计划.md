...
### 里程碑 B（1~2 周）：美术替换与“质感层”上线 [已完成]

- [x] Welcome/Start 全面换皮：背景、字体、按钮风格统一
...
- [x] Turn-Based Battle（回合制交互）界面终端化：统一配色/按钮/信息密度
- [x] Inventory/Equipment 完成物品卡体系：稀有度框、图标位、属性密度控制
- [x] 产出验收：
  - [x] 关键界面截图 10 张对比（改造前/后）
  - [x] 视觉一致性检查：颜色/字号/间距/阴影/边框统一

### 里程碑 C（1 周）：内容展示升级（仍不改数值）[进行中]

- [x] 事件呈现：为“探索结果/战斗结果/突破结果”增加结构化 UI（标题、风险、奖励、按钮）—— 日志面板已升级为结构化卡片
- [ ] 关键系统增强反馈：掉落浮字、升级动效、低血量/低寿命/高辐射提示
- [ ] 产出验收：
  - [x] 日志不再是“纯文字堆叠”，而是信息可扫描（颜色、标签、图标、分组）
