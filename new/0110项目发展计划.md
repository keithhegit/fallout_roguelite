...
**下一阶段关键任务**：
1.  **工程质量**（优先级 P1）：ESLint 警告清零。
2.  **视觉细节打磨**（优先级 P2）：进一步优化各个 Modal 的细节。
...
- **目标**：让装备对比和管理更直观。
- **任务**：
    - [x] 优化 `ItemCard` 布局，支持紧凑模式。
    - [x] 增强 `CompareTooltip`，清晰展示属性升降（绿升红降）。
...
| **P1** | 地点美术素材实装 (剩余 40+) | 2h | ✅ 完成 |
| **P1** | Inventory 物品卡优化 | 3h | ✅ 完成 |
| **P2** | 结构化日志升级 | 4h | ✅ 完成 |
| **P2** | ESLint 警告清零 | 2h | ✅ 完成 |
...
| 日期 | 版本 | 变更说明 |
|-----|------|---------|
| 2026-01-11 | v2.2.2 | ESLint 警告清理 (Hooks/Utils/Services)，Inventory 紧凑模式实装 |
...
## 6. 测试清单 (Testing Checklist)

为了确保 v2.2.2 版本稳定性，请重点测试以下模块：

### 6.1 核心功能 (Core Features)
- [ ] **Inventory (物品栏)**
    - [ ] 切换 **DENSE / DETAIL** 视图，检查布局是否正常。
    - [ ] 鼠标悬停物品，检查 **Tooltip** 对比功能（绿色上升箭头/红色下降箭头）。
    - [ ] 执行 **Batch Discard** (批量丢弃) 和 **Batch Use** (批量使用)，确保无报错。
- [ ] **Battle (战斗系统)**
    - [ ] 进入战斗，检查日志输出是否清晰。
    - [ ] 使用物品/技能，确保无 `undefined` 错误（修复了 `battleService.ts` 类型）。
    - [ ] 战斗胜利/失败结算界面是否正常显示。
- [ ] **Grotto (洞府系统)**
    - [ ] 尝试 **Upgrade Grotto** (升级洞府)，检查资源扣除和等级变化。
    - [ ] 种植/收获药草，确保倒计时和收获数量正确。

### 6.2 数据与存档 (Data & Save)
- [ ] **Save/Load (存档)**
    - [ ] 手动保存游戏，刷新页面后读取，确保 `playTime` 和 `inventory` 数据无丢失。
    - [ ] 检查 `GameHeader` 的在线人数 (`PartyKit`) 是否显示正常。

### 6.3 视觉体验 (Visuals)
- [ ] **UI Theme**
    - [ ] 检查各个 Modal (Inventory, Character, etc.) 的边框和背景是否统一为 Fallout 风格。
    - [ ] 确认没有残留的 `mystic-*` 样式或亮色系修仙风格元素。
